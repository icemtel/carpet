"""
2019-08-30: lattice_triangular2

2019-10-30: - Get tol as parameter
            - create a subfolder for results
            - change fixed points dictionary name

2020-01-27: - just change parameters: 6x6 carpet, changed Fourier truncation order
            - change fixpoint_dict location
            - change lattice back to non-rotated!

2020-01-27: - rot. lattice 6x6

2020-02-19: scipy -> numpy; carpet - updated
2020-02-25: next-to-nearest neighbours
script takes arguments
- k1, k2, delta0, tol

- Take fixed points data from fixed point dictionary file
- Compute matrix L and eigenvalues for the fixpoint, located, corresponding to (k1,k2)-twist
- Create a subfolder and save the result
"""
import sys
import logging
import os
...
NN, TT = lattice.get_neighbours_list(coords, nx, ny, a, distances=(1, np.sqrt(3)))  # get list of neighbours and relative positions
...
# Load fixpoints
filename = '../fixpoint_dict_nx=6_ny=6_tol=1.000E-08.pkl' # 'fixpoint_dict.pkl'
...
output_folder = 'L_delta0={:.3E}_tol={:.3E}/'.format(delta0, tol)
os.makedirs(output_folder, exist_ok=True)
filename = output_folder + "/L_mtwist_k1={}_k2={}.pkl".format(k1,k2)
