...
import numpy as np
import pickle
from sim_setup import N, solve_cycle, solve_cycle_back, get_fixpoint
import carpet
...
def rand_sphere(n):  # get random n-dimensional vector; distributed uniformly on n-1-dim sphere
    u = np.random.normal(0, 1, n)  # an array of d normally distributed random variables
    d = np.sum(u ** 2) ** (0.5)
    return u / d
...
def define_get_pert(n):
    ...
    def get_pert(eps):
        v = rand_sphere(n - 1)  # get (uniformly) random vector on n-2 dimensional sphere
        v = np.concatenate(([0], v))  # embed the sphere in the (n-1)-hyperplane in n-dimensional space
